...
class theme_setup_netflibs {
	...
	/**
	 * theme_setup_netflibs constructor.
	 */
	...
	public function admin_menu() {
		add_submenu_page(
			$this->parent_slug,
			__('ثبت لایسنس', $this->text_domain),
			__('ثبت لایسنس', $this->text_domain),
			'manage_options',
			$this->slug,
	...
	public function menu_content() {
        ...
        <div class="wrap wpmlr_wrap" data-show="<?php echo $data_show ?>">
            <h1><?php _e('ثبت لایسنس', $this->text_domain); ?></h1>
			<?php if( isset($now) && !empty($now) ): ?>
                <p><?php _e('شما قبلاً کلید مجوز خود را ثبت کرده اید. برای تایید مجدد آن، می توانید از این فرم استفاده کنید.', $this->text_domain); ?></p>
                <div class="current_license">
                    <span class="current_label"><?php _e('مجوز فعلی شما:', $this->text_domain); ?></span>
                    ...
                    <div class="keys">
                        <a href="#" class="button button-primary wpmlr_revalidate" data-key="<?php echo $starter; ?>"><?php _e('اعتبار مجدد', $this->text_domain); ?></a>
                        <a href="#" class="button zhk_guard_new_key"><?php _e('مجوز دیگری را حذف و ارسال کنید', $this->text_domain); ?></a>
                    </div>
                    ...
                    <form action="#" method="post" class="register_version_form">
                        ...
                        <button class="button button-primary"><?php _e('ثبت لایسنس', $this->text_domain); ?></button>
                        <div class="result">
	...
	public function wp_starter() {
		...
		if ($result->status=='successful') {
			...
			$result = array(
				...
				'action' => 1,
				'message' => __('کد مجوز معتبر است.', $this->text_domain),
				'timer' => time(),
			);
     ...
     * If you want add an interrupt in your plugin or theme simply can use this static method: theme_setup_netflibs::is_activated
	 ...
	 * @param $settings
	 *
	 * @return null|theme_setup_netflibs
	 */
...
function zhk_guard_init() {
	$settings = [
		'name'          => 'NETFLIBS',
		'slug'          => 'NETFLIBS-license',
		'parent_slug'   => 'index.php', // Read this: https://developer.wordpress.org/reference/functions/add_submenu_page/#parameters
		'text_domain'   => '',
		'product_token' => 'a6fea451-b7ca-4cd4-8ae8-27d059f8be12', // Get it from here: https://zhaket.com/dashboard/licenses/
		'option_name'   => 'NEF_guard_register_OPtion'
	];
	theme_setup_netflibs::instance($settings);
}